...
If we only care about the min we can do so by removing the max (but leaving the comma), so `"a{7,}"` checks that there are 7 `a`'s in a row at least. If we want an exact number of occurrences we remove the comma too, so `"a{5}"` checks that there are 5 `a`'s in a row exactly.
